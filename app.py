...
def get_map(country_dest, country_now):
    ...
    if prediction_polygon.geom_type == 'Polygon':
        ...
        folium.Polygon(locations=all_points, style_function=lambda x: {'color': 'green'}).add_to(m)
    else:
    ...
    folium.Polygon(locations=all_points).add_to(m)
    cur_country_poly = world.loc[world['name'] == country_now]['geometry'].values[0]
    cur_country_center = cur_country_poly.centroid.coords[0]
    folium.PolyLine(
    ...
    folium.Marker(
        location=[cur_country_center[1], cur_country_center[0]], tooltip="Start", icon=folium.Icon(color='blue', icon='play'),
    ).add_to(m)
    return m
...
FLIGHT_LENGTH = st.sidebar.slider("How long are you ready to fly?", 1, 12)

SIGHTSEEING_ATT = st.sidebar.slider(
    """Your attitude to sightseeing: 1 - Definitely not, 10 - Can't live without sightseeing""", 1, 10)
...
if submit_btn:
    ...
    folium_static(m)
    st.markdown('<p class="big-font">' + "Also you can visit: " + advice + '</p>', unsafe_allow_html=True)
